...
def generate_sgp_configs(board_size, num_geoms, complexity_min_max, complexity_bin_size, shapes, colors):
    ...
    complexity_bins = {
        c1: {c2: 0 for c2 in range(complexity_min_max["c2"]["min"], complexity_min_max["c2"]["max"] + 1)}
    ...
    while True:
        ...
        if (complexity['c1'] not in complexity_bins or  # Check if c1 exists in the outer dictionary
            complexity['c2'] not in complexity_bins[complexity['c1']] or  # Check if c2 exists in the inner dictionary
            complexity_bins[complexity['c1']][complexity['c2']] >= complexity_bin_size):  # Check if the bin size is exceeded
            continue
