...
| D-007 | Binary hygiene: in-repo assets posture — authored Unity assets under `unity/` and docs media under `docs/assets/` only (plain git, no LFS); generated artifacts (WebGL builds, run outputs, datasets, weights) never enter history — app builds ship as GitHub Release assets; wired through the single `.claude/asset-dirs.txt` seam so hook and CI can never disagree | ✅ | [ADR-0007](adr-0007-binary-hygiene.md) |
| D-008 | Python package layout: one installable src-layout package `ivispar` under `python/`; subpackages mirror the runner's architecture seams, held honest by an import smoke test; entry points land with the framework wave | ✅ | [ADR-0008](adr-0008-python-package-layout.md) |
