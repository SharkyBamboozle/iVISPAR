...
class LLMAgent:
    ...
    def parse_action(self, response, split_at="action: move "):
        ...
        if split_index != -1:
            ...
            action = "move " + cleaned_response[
                               split_index + len(split_at):].strip()  # Extract text after the last occurrence
        else:
    ...
    def act(self, observation, loop_iteration):
        ...
        if sceneUnderstanding:
            try:
                ...
                content = [
                    {"type": "text",
                     "text": """In this image with blue background you will see the board, describe the board state. Only describe the objects with their coordinates, don't describe empty fields."""},
        ...
        if isinstance(observation, Image.Image):
            try:
                ...
                if self.max_history > 0:
                    for i, prev_exchange in enumerate(self.chat_history[-self.max_history:], 1):
                        ...
                        messages.append({
                            ...
                            "content": [
                                {"type": "image",
                                 "source": {"type": "base64", "media_type": "image/png", "data": prev_image_base64}}
                            ]
                        })
                ...
                messages.append({
                    ...
                    "content": [
                        {"type": "image",
                         "source": {"type": "base64", "media_type": "image/png", "data": current_base64}}
                    ]
                })
    ...
    def act(self, observation, loop_iteration):
        if isinstance(observation, Image.Image):
            try:
                ...
                formatted_message = format_message_structure_gemini(
                    messages)  # use it for debugging make sure the input is correct
                self.episode_logger.info(formatted_message)
...
class OpenSourceAgent:
    def __init__(self, model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus,
                 COT=False):
        self.n_gpus = n_gpus
    ...
    def act(self, observation, loop_iteration=None):
        try:
            ...
            sampling_params = SamplingParams(temperature=1.0, top_p=0.95, top_k=50, max_tokens=500,
...
class Qwen2VLAgent(OpenSourceAgent):
    def __init__(self, model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus,
        ...
        self.model = LLM(model_path, tensor_parallel_size=n_gpus, enforce_eager=n_gpus > 4,
                         limit_mm_per_prompt={"image": max_history + 2}, trust_remote_code=True)
...
class LlamaAgent(OpenSourceAgent):
    def __init__(self, model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus,
        ...
        super().__init__(model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus, COT)
        self.model = LLM(model_path, tensor_parallel_size=n_gpus, enforce_eager=n_gpus > 4,
                         limit_mm_per_prompt={"image": max_history + 2}, trust_remote_code=True, cpu_offload_gb=10,
                         swap_space=10)
...
class DeepseekAgent(OpenSourceAgent):
    def __init__(self, model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus,
        ...
        super().__init__(model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus, COT)
        self.model = LLM(model_path, tensor_parallel_size=n_gpus, enforce_eager=n_gpus > 4,
                         limit_mm_per_prompt={"image": max_history + 2},
                         hf_overrides={"architectures": ["DeepseekVLV2ForCausalLM"]}, trust_remote_code=True)
...
class InternVLAgent(OpenSourceAgent):
    def __init__(self, model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus,
        ...
        super().__init__(model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus, COT)
        self.model = LLM(model_path, tensor_parallel_size=n_gpus, enforce_eager=n_gpus > 4,
                         limit_mm_per_prompt={"image": max_history + 2}, mm_processor_kwargs={"max_dynamic_patch": 4},
