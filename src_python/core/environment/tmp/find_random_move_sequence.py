...
def generate_random_valid_path(n, initial_state, max_steps=100):
    ...
    path = [initial_state.tolist()]  # Initial checkpoint_state is the first checkpoint_state in the path
...
def generate_random_invalid_path(n, initial_state, max_steps=100):
    ...
    the current checkpoint_state is repeated for the next step, but the invalid move is still recorded.
    ...
    initial_state = np.array(initial_state)
    path = [initial_state.tolist()]  # Initial checkpoint_state is the first checkpoint_state in the path
    valid_state = initial_state.copy()  # The checkpoint_state that we revert to if the move is invalid
    ...
    for step in range(max_steps):
        ...
        # Create the new potential checkpoint_state
        ...
        if 0 <= new_x < n and 0 <= new_y < n and (new_x, new_y) not in map(tuple, valid_state):
            valid_state = new_state.copy()  # Accept the move and update the valid checkpoint_state

        # Record the new checkpoint_state, even if it's invalid
        path.append(new_state.tolist())  # Always add the new checkpoint_state, even if invalid
