...
def calculate_manhattan_heuristic(initial_states, goal_states):
    ...
    #Todo: advanced for rotation addrmv and spin actions
...
def get_neighbors(state, n):
    """
    Generate all valid neighboring states for a given checkpoint_state.
    ...
    for i, (x, y) in enumerate(state):
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:  # Possible moves: up, down, left, right
            ...
            if 0 <= new_x < n and 0 <= new_y < n and (new_x, new_y) not in tiles_set:
                # Generate a new checkpoint_state by moving the current tile
...
def generate_starting_state(goal_state, action_set, shortest_path, maxsteps=1000):
    # TODO: validate length of c1 path solution
    return_state = np.zeros(10)
    return return_state
...
def a_star(n, initial_state, goal_state, max_depth=None):
    ...
    while open_set:
        # Get the checkpoint_state with the lowest f_score
        current_f_score, current_tuple = heapq.heappop(open_set)
        current_state = np.array(current_tuple)

        # If the current checkpoint_state is the goal checkpoint_state, reconstruct the path
        if np.array_equal(current_state, goal_state):
            ...
            path.append([[int(coord) for coord in pair] for pair in tuple(map(tuple, initial_state))])  # Add initial checkpoint_state
            return path[::-1]
...
def find_config_by_random_expand(n, goal_state, path_length, max_steps=100):
    ...
    from the initial checkpoint_state to the goal checkpoint_state.

    Args:
        n (int): The board size (n x n).
        goal_state (list): The goal checkpoint_state of the puzzle (list of [x, y] pairs).
        path_length (int): The required distance (in optimal moves) from the generated initial checkpoint_state to the goal checkpoint_state.
        max_steps (int, optional): Maximum number of backward moves to try before stopping.

    Returns:
        list: The generated initial checkpoint_state, or None if no valid checkpoint_state was found.
    """
    ...
    for step in range(max_steps):
        ...
        if current_path is None:
            ...
            break
        current_path_length = len(current_path) - 1  # Subtract 1 since the first checkpoint_state doesn't count as a step

        #print(f"Step {step + 1}: Current path length from checkpoint_state to goal is {current_path_length}")

        # Get all possible neighbors of the current checkpoint_state
        neighbors = get_neighbors(current_state, n)
        random.shuffle(neighbors)  # Randomize the neighbor selection process

        valid_next_state = None  # Track if we find a valid next checkpoint_state
        for neighbor in neighbors:
            neighbor_path = a_star(n, neighbor, goal_state, max_depth=path_length)

            if neighbor_path is not None:
                neighbor_path_length = len(neighbor_path) - 1  # Subtract 1 since the first checkpoint_state doesn't count as a step
                if neighbor_path_length > current_path_length:  # Check if the neighbor's path is longer than the current
                    ...
                    #print(f"  Found a valid next checkpoint_state with path length {neighbor_path_length} (previous was {current_path_length})")
                    break  # We only need one valid neighbor

        if valid_next_state is not None:
            current_state = valid_next_state  # Move to the new checkpoint_state
        else:
            pass
            #print(f"Step {step + 1}: No valid backward step found from checkpoint_state {current_state}")

        # If the current checkpoint_state's path to the goal has the desired path length, return it
