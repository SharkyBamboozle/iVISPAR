...
/*This class defines the grid board based on the Grid object it holds */
{
    ...
    public void SetObjectUID(int x, int z,int uid)
    {
        grid.SetObjectUID(x,z,uid);
    }
    public int GetObjectUID(int x, int z)
    {
        return grid.GetObjectUID(x,z);
    }
    void Awake()
    /*Gets called when Environment is set up and initializes the grid*/
    {
        ...
        {
            ExperimentData landmarkData = ExperimentManager.Instance.loadedExperimentData;  // Use the shared class
