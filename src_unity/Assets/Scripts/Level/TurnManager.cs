...
//using Unity.VisualScripting.Dependencies.Sqlite; //threw error in build process and seems not to be used?
using UnityEngine;
...
public class TurnManager : MonoBehaviour
{
    ...
    string[] knownRotationDirections = { "top-left","bottom-left","top-right","bottom-right","up-left","down-left","up-right","down-right"};
    string[] knownObjects = { "cube", "tile", "sphere", "cylinder", "pyramid", "cone", "prism", "diamond" };
    string[] knownCommands = { "move","add","delete","flip","rotate" , "start", "reset","done" };
    void Start()
    {
    ...
    public Tuple<string,string,string,string,int> decodeCommand(string command)
    {
        ...
        if(cmd != "rotate")
            ...
            foreach (var obj in knownRotationDirections)
            {
                if (command.Contains(obj))
                {
                    foundDirection = obj;
    ...
    {    
        ...
        foreach (string command in turnCommands)
        {
            ...
            if(decodedText.Item1.Equals("move"))
            {
            ...
            if(decodedText.Item1.Equals("rotate"))
            {
                ...
                for(int i = 0 ; i < decodedText.Item5; i++)
                {
                    ...
                    EventHandler.Instance.InvokeCommand("rotate",id,decodedText.Item4);

                }               
            }
            else if(decodedText.Item1.Equals("add") || decodedText.Item1.Equals("delete"))
            {
                Debug.Log("processing command Queue: " +command.ToString());
                ...
                EventHandler.Instance.InvokeCommand("addremove",id,decodedText.Item1);  
            }
            else if(decodedText.Item1.Equals("flip"))
            {
                Debug.Log("processing command Queue: " +command.ToString());
                int id = 0;
                ...
                for(int i = 0 ; i < decodedText.Item5; i++)
                {
                    ...
                    EventHandler.Instance.InvokeCommand("flip",id,decodedText.Item4);

                }              
            }
            else if(decodedText.Item1.Equals("start"))
            {
        
                EventHandler.Instance.InvokeCommand("init_target");
                Debugger.Instance.SetValidity("valid command. start of experiment");
            }
            else if(decodedText.Item1.Equals("done"))
            {
            ...
            else if(decodedText.Item1.Equals("reset"))
            {
                if(ExperimentManager.Instance.humanExperiment)
    ...
    public void ResponseAck(DataPacket response)
    {
        ...
        if(recievedDoneCommand || ExperimentManager.Instance.loadedExperimentData.auto_done_check)
        {
            ...
            {
                if(!ExperimentManager.Instance.loadedExperimentData.auto_done_check)
